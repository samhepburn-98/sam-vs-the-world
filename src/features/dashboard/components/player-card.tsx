...
function useCardUnit() {
  ...
  useEffect(() => {
    ...
    if (!el) return
    const update = () => el.style.setProperty("--u", `${el.clientWidth / 100}px`)
    update()
...
export function PlayerCard({
  ...
  const stat = (a: PlayerAttribute) => {
    const pair = (
      ...
      >
        <dd
          className="text-right font-extrabold tabular-nums text-white"
          style={{ fontSize: u(7.8) }}
        >
          {a.display}
        </dd>
        <dt className="font-medium" style={{ fontSize: u(7.8), color: t.muted }}>
          {a.code}
            ...
            >
              <HandIcon
                ...
                style={{
                  ...
                  height: u(5.4),
                  transform:
                    handedness === "left" ? "scaleX(-1)" : undefined,
                }}
              />
              {handedness === "left" ? "L" : "R"}
