...
| ~~`GameCard`~~ | *never built* — `ScoreStrip` and `GameScoreChart` cover match detail and the summary between them | — | — |
...
Whatever is in `src/components/ui`, themed by tokens only and never edited for one screen. The
current set is in Storybook under **Primitives** — that list is generated by what exists, so it
can't go stale the way an inventory typed here would.
